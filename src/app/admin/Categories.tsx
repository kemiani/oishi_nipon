...
export default function Categories() {
  ...
  const handleCreate = async () => {
    ...
    if (trimmedName.length < 2) { setError('Mínimo 2 caracteres'); return; }
    if (trimmedName.length > 100) { setError('Máximo 100 caracteres'); return; }
    ...
    try {
      ...
      if (error) { setError('Error al crear la categoría'); console.error(error); }
      else { setNewCategory(''); await fetchCategories(); }
    } catch (err) {
      setError('Error inesperado'); console.error(err);
  ...
  return (
    <section className="glass-card p-6">
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-500">{newCategory.length}/100 caracteres</p>
      </div>
    </section>
  );
}